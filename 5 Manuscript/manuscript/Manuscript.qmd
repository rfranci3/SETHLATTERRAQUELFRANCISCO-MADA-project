...
Evaluate both antimicrobial gene resistence diversity and burden in white storks that utilize landfills and natural areas over the breeding season of years 2020 to 2021.
...
Linear mixed models (LMMs) were used to predict total antimicrobial gene burden in white storks, with nest identification again classified as a random effects parameter. The global model, containing the variables landfill use index, age, sample period, and nest success, was the highest performing model (Table 3). In this model, each predictor variable was positively correlated with antimicrobial gene burden (Table 4). The next highest performing model contained age as a single predictor, with a model performance score of 86.39%. Similarly to the top analysis, several models where overfit likely due to similar reasons.
...
-   As multi-drug resistance increases throughout the breeding season, mean LUI decreases and nest success decreases. There is a positive effect between increased LUI and nest success [@Lopez-Garcia2021]. Intuitively this makes some sense due to MDR being associated with increased anthropogenic exposure, heavy metal exposure, and increased virulence of pathogenic bacteria. More target research would be needed to better understand how MDR may effect nest success long-term.

-   Resistance gene burden appears to increase as mean LUI decreases and age increases. Most of the burden is due to blaTEM, a common resistance gene associated with beta-lactam resistance. Beta-lactam resistance is one of the most studied AMRs in wildlife. Drugs within the Beta-lactam class are often first line antibiotics, such as penicillin, and are active against many gram-positive, gram-negative, and anaerobic organisms. At this point, beta-lactam resistance is thought to be prevalent on most landscapes due to human or livestock encroachment thus is often used as a measure of anthropogenic impact. While sampling periods did not explain burden, the top blaTEM model did show a trend in burden increasing from the 2nd sampling period to the 3rd sampling period.This may be related to shear increase of foraging required to feed ~3 to 4 kilogram fledglings.This foraging could have occurred in areas other then the landfill, such as neighboring cattle pastures which may also be sources for ARGs.

## Strengths and Limitations

-   While none of our models resulted in statistically significant data, valuable trends can be seen in respect to the relationship between breeding season events, AMR trends, and landfill use. Hopefully, these trends can foster future research to consider expandings beyond the hypothesis that landfills are the risky place for wildlife to acquire ARGs.

-   The dataset utilized for the models was much smaller then previously envisioned due to COVID-19 barring landfill data collection throughout the entirety of 2020. Perhaps with more observations, the results of the models may have been stronger and variables may have been more normally distributed. Moving forward, it may be best to transform our standardized variables using a log transformation to attempt to achieve a more normal distribution for analysis, especially in regard to our GLMMs.
